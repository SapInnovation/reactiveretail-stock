...
This can be done by creating a ```@Configuration``` class ```StreamsConfig``` with below code:
...
Create a simple ```Greetings``` class with below code that will represent the message object we read from and write to the ```greetings``` Kafka topic:
...
Let's create the ```GreetingsService``` class with below code that will write a ```Greetings``` object to the ```greetings``` Kafka topic:
...
Let's create a ```GreetingsListener``` class that will listen to messages on the ```greetings``` Kafka topic and log them on the console:
...
The last piece of the puzzle is the ```StreamKafkaApplication``` class that was auto-generated by the Spring Initializer:
